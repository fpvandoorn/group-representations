...
variables (i : V →ₗ[monoid_algebra k G] W)

def is_zero_map : Prop := ∀ v : V, i v = 0

lemma check_zero : is_zero_map (0 • i) := begin unfold is_zero_map, intro, simp [i.map_smul] end

def invariant_submodule (N : submodule (monoid_algebra k G) W) : Prop :=
  ∀ v : W, ∀ g : G, (group_smul.linear_map k W g) v = v

def irreducible (M : Type u) [add_comm_group M] [module (monoid_algebra k G) M] : Prop :=
∀ N : submodule (monoid_algebra k G) M, N = ⊥ ∨ N = ⊤

#print irreducible

lemma schur (i : V →ₗ[monoid_algebra k G] W) : irreducible V ∧ irreducible W → (function.bijective i ↔ ¬ is_zero_map i) := 
sorry
