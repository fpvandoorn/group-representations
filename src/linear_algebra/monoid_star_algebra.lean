...
We take the measure to be the sum over all elements.  This makes the convolution product
definable by restricting the sum ∑_x,y f(x) g(y) to x*y = a.

Ideally we would take the definitions from monoid_algebra, interpreted formally, and
check the summability conditions.  This requires generalizing finsupp to summable or some
stronger norm condition (we could also use ℓ_2). 

NOTE: we are trying to use summable as defined in topology.algebra.infinite_sum but this is 
NOT the same as ℓ_1 whose definition involves the sum of the norm of the elements.
So we would rather use l1_space but using the discreteness to define the convolution.
...
Similarly, I attempted to just define `add_monoid_star_algebra k G := monoid_star_algebra k (multiplicative G)`,
...
section
variables [add_comm_monoid k] [topological_space k] [monoid G]
...
-- this is not right but we want a similar class of summable functions.
@[derive [inhabited, add_comm_monoid]]
def monoid_star_algebra : Type (max u₁ u₂) := G →₀ k

end

namespace monoid_star_algebra

variables {k G}
local attribute [reducible] monoid_star_algebra

section
variables [add_comm_monoid k] [topological_space k] [monoid G]

/-- The product of `f g : monoid_star_algebra k G` is the summable function
  whose value at `a` is the sum of `f x * g y` over all pairs `x, y`
  such that `x * y = a`. (Think of the group ring of a group.) -/

def mul_def1 (f g : monoid_star_algebra k G) := (f.sum $ λa₁ b₁, g.sum $ λa₂ b₂, single (a₁ * a₂) (b₁ * b₂)) 

#check mul_def1 

lemma mul_convergence (f g : monoid_star_algebra k G) (hf : summable f) (hg : summable g) : summable (mul_def1 f g) := 
begin unfold summable at *, unfold mul_def1, rcases hf, rcases hg, 
end

instance : has_mul (monoid_star_algebra k G) :=
⟨λf g, f.sum $ λa₁ b₁, g.sum $ λa₂ b₂, single (a₁ * a₂) (b₁ * b₂)⟩

lemma mul_def {f g : monoid_star_algebra k G} :
  f * g = (f.sum $ λa₁ b₁, g.sum $ λa₂ b₂, single (a₁ * a₂) (b₁ * b₂)) :=
rfl

lemma mul_apply (f g : monoid_star_algebra k G) (x : G) :
...
lemma mul_apply_antidiagonal (f g : monoid_star_algebra k G) (x : G) (s : finset (G × G))
...
lemma support_mul (a b : monoid_star_algebra k G) :
...
instance : has_one (monoid_star_algebra k G) :=
⟨single 1 1⟩

lemma one_def : (1 : monoid_star_algebra k G) = single 1 1 :=
rfl

-- TODO: the simplifier unfolds 0 in the instance proof!
protected lemma zero_mul (f : monoid_star_algebra k G) : 0 * f = 0 :=
by simp only [mul_def, sum_zero_index]

protected lemma mul_zero (f : monoid_star_algebra k G) : f * 0 = 0 :=
by simp only [mul_def, sum_zero_index, sum_zero]

private lemma left_distrib (a b c : monoid_star_algebra k G) : a * (b + c) = a * b + a * c :=
...
private lemma right_distrib (a b c : monoid_star_algebra k G) : (a + b) * c = a * c + b * c :=
...
instance : semiring (monoid_star_algebra k G) :=
{ one       := 1,
  mul       := (*),
  ...
  zero_mul  := monoid_star_algebra.zero_mul,
  mul_zero  := monoid_star_algebra.mul_zero,
...
@[simp] lemma single_mul_single {a₁ a₂ : G} {b₁ b₂ : k} :
  (single a₁ b₁ : monoid_star_algebra k G) * single a₂ b₂ = single (a₁ * a₂) (b₁ * b₂) :=
...
@[simp] lemma single_pow {a : G} {b : k} :
  ∀ n : ℕ, (single a b : monoid_star_algebra k G)^n = single (a^n) (b ^ n)
| 0 := rfl
...
def of : G →* monoid_star_algebra k G :=
...
lemma mul_single_apply_aux (f : monoid_star_algebra k G) {r : k}
...
lemma mul_single_one_apply (f : monoid_star_algebra k G) (r : k) (x : G) :
  (f * single 1 r) x = f x * r :=
f.mul_single_apply_aux $ λ a, by rw [mul_one]

lemma single_mul_apply_aux (f : monoid_star_algebra k G) {r : k} {x y z : G}
  (H : ∀ a, x * a = y ↔ a = z) :
...
lemma single_one_mul_apply (f : monoid_star_algebra k G) (r : k) (x : G) :
  (single 1 r * f) x = r * f x :=
f.single_mul_apply_aux $ λ a, by rw [one_mul]

end

instance [comm_semiring k] [comm_monoid G] : comm_semiring (monoid_star_algebra k G) :=
{ mul_comm := assume f g,
  begin
  ...
  .. monoid_star_algebra.semiring }

instance [ring k] : has_neg (monoid_star_algebra k G) :=
by apply_instance

instance [ring k] [monoid G] : ring (monoid_star_algebra k G) :=
{ neg := has_neg.neg,
  add_left_neg := add_left_neg,
  .. monoid_star_algebra.semiring }

instance [comm_ring k] [comm_monoid G] : comm_ring (monoid_star_algebra k G) :=
{ mul_comm := mul_comm, .. monoid_star_algebra.ring}

instance [semiring k] : has_scalar k (monoid_star_algebra k G) :=
finsupp.has_scalar

instance [semiring k] : semimodule k (monoid_star_algebra k G) :=
...
instance [comm_semiring k] [monoid G] : algebra k (monoid_star_algebra k G) :=
{ to_fun    := single 1,
  map_one'  := rfl,
...
@[simp] lemma coe_algebra_map [comm_semiring k] [monoid G] :
  (algebra_map k (monoid_star_algebra k G) : k → monoid_star_algebra k G) = single 1 :=
rfl

lemma single_eq_algebra_map_mul_of [comm_semiring k] [monoid G] (a : G) (b : k) :
  single a b = (algebra_map k (monoid_star_algebra k G) : k → monoid_star_algebra k G) b * of k G a :=
by simp

instance [group G] [semiring k] :
  distrib_mul_action G (monoid_star_algebra k G) :=
...
`monoid_star_algebra k G →ₐ[k] R`. -/
def lift : (G →* R) ≃ (monoid_star_algebra k G →ₐ[k] R) :=
{ inv_fun := λ f, (f : monoid_star_algebra k G →* R).comp (of k G),
...
lemma lift_apply (F : G →* R) (f : monoid_star_algebra k G) :
  lift k G R F f = f.sum (λ a b, b • F a) := rfl

@[simp] lemma lift_symm_apply (F : monoid_star_algebra k G →ₐ[k] R) (x : G) :
...
lemma lift_unique' (F : monoid_star_algebra k G →ₐ[k] R) :
  F = lift k G R ((F : monoid_star_algebra k G →* R).comp (of k G)) :=
((lift k G R).apply_symm_apply F).symm

/-- Decomposition of a `k`-algebra homomorphism from `monoid_star_algebra k G` by
its values on `F (single a 1)`. -/
lemma lift_unique (F : monoid_star_algebra k G →ₐ[k] R) (f : monoid_star_algebra k G) :
  F f = f.sum (λ a b, b • F (single a 1)) :=
by conv_lhs { rw lift_unique' F, simp [lift_apply] }

/-- A `k`-algebra homomorphism from `monoid_star_algebra k G` is uniquely defined by its
values on the functions `single a 1`. -/
lemma alg_hom_ext ⦃φ₁ φ₂ : monoid_star_algebra k G →ₐ[k] R⦄
...
def group_smul.linear_map [group G] [comm_ring k]
  (V : Type u₃) [add_comm_group V] [module (monoid_star_algebra k G) V] (g : G) :
...
lemma group_smul.linear_map_apply [group G] [comm_ring k]
  (V : Type u₃) [add_comm_group V] [module (monoid_star_algebra k G) V] (g : G) (v : V) :
...
variables [group G] [comm_ring k]
  {V : Type u₃} {gV : add_comm_group V} {mV : module (monoid_star_algebra k G) V}
  {W : Type u₃} {gW : add_comm_group W} {mW : module (monoid_star_algebra k G) W}
  (f : (module.restrict_scalars k (monoid_star_algebra k G) V) →ₗ[k]
       (module.restrict_scalars k (monoid_star_algebra k G) W))
...
@[simp]
lemma mul_single_apply (f : monoid_star_algebra k G) (r : k) (x y : G) :
  (f * single x r) y = f (y * x⁻¹) * r :=
f.mul_single_apply_aux $ λ a, eq_mul_inv_iff_mul_eq.symm

@[simp]
lemma single_mul_apply (r : k) (x : G) (f : monoid_star_algebra k G) (y : G) :
  (single x r * f) y = r * f (x⁻¹ * y) :=
f.single_mul_apply_aux $ λ z, eq_inv_mul_iff_mul_eq.symm

lemma mul_apply_left (f g : monoid_star_algebra k G) (x : G) :
...
lemma mul_apply_right (f g : monoid_star_algebra k G) (x : G) :
...
-/
@[derive [inhabited, add_comm_monoid]]
def add_monoid_star_algebra := G →₀ k

end

namespace add_monoid_star_algebra

variables {k G}
local attribute [reducible] add_monoid_star_algebra

section
variables [semiring k] [add_monoid G]

/-- The product of `f g : add_monoid_star_algebra k G` is the finitely supported function
  whose value at `a` is the sum of `f x * g y` over all pairs `x, y`
  such that `x + y = a`. (Think of the product of multivariate
  polynomials where `α` is the additive monoid of monomial exponents.) -/
instance : has_mul (add_monoid_star_algebra k G) :=
⟨λf g, f.sum $ λa₁ b₁, g.sum $ λa₂ b₂, single (a₁ + a₂) (b₁ * b₂)⟩

lemma mul_def {f g : add_monoid_star_algebra k G} :
  f * g = (f.sum $ λa₁ b₁, g.sum $ λa₂ b₂, single (a₁ + a₂) (b₁ * b₂)) :=
rfl

lemma mul_apply (f g : add_monoid_star_algebra k G) (x : G) :
  (f * g) x = (f.sum $ λa₁ b₁, g.sum $ λa₂ b₂, if a₁ + a₂ = x then b₁ * b₂ else 0) :=
begin
  rw [mul_def],
  simp only [finsupp.sum_apply, single_apply],
...
lemma support_mul (a b : add_monoid_star_algebra k G) :
  (a * b).support ⊆ a.support.bind (λa₁, b.support.bind $ λa₂, {a₁ + a₂}) :=
subset.trans support_sum $ bind_mono $ assume a₁ _,
  subset.trans support_sum $ bind_mono $ assume a₂ _, support_single_subset
...
instance : has_one (add_monoid_star_algebra k G) :=
⟨single 0 1⟩

lemma one_def : (1 : add_monoid_star_algebra k G) = single 0 1 :=
rfl

-- TODO: the simplifier unfolds 0 in the instance proof!
protected lemma zero_mul (f : add_monoid_star_algebra k G) : 0 * f = 0 :=
by simp only [mul_def, sum_zero_index]

protected lemma mul_zero (f : add_monoid_star_algebra k G) : f * 0 = 0 :=
by simp only [mul_def, sum_zero_index, sum_zero]

private lemma left_distrib (a b c : add_monoid_star_algebra k G) : a * (b + c) = a * b + a * c :=
by simp only [mul_def, sum_add_index, mul_add, mul_zero, single_zero, single_add,
  eq_self_iff_true, forall_true_iff, forall_3_true_iff, sum_add]

private lemma right_distrib (a b c : add_monoid_star_algebra k G) : (a + b) * c = a * c + b * c :=
by simp only [mul_def, sum_add_index, add_mul, mul_zero, zero_mul, single_zero,
  single_add, eq_self_iff_true, forall_true_iff, forall_3_true_iff, sum_zero, sum_add]

instance : semiring (add_monoid_star_algebra k G) :=
{ one       := 1,
  mul       := (*),
  one_mul   := assume f, by simp only [mul_def, one_def, sum_single_index, zero_mul,
    single_zero, sum_zero, zero_add, one_mul, sum_single],
  mul_one   := assume f, by simp only [mul_def, one_def, sum_single_index, mul_zero,
    single_zero, sum_zero, add_zero, mul_one, sum_single],
  zero_mul  := add_monoid_star_algebra.zero_mul,
  mul_zero  := add_monoid_star_algebra.mul_zero,
  mul_assoc := assume f g h, by simp only [mul_def, sum_sum_index, sum_zero_index, sum_add_index,
    sum_single_index, single_zero, single_add, eq_self_iff_true, forall_true_iff, forall_3_true_iff,
    add_mul, mul_add, add_assoc, mul_assoc, zero_mul, mul_zero, sum_zero, sum_add],
  left_distrib  := left_distrib,
...
lemma single_mul_single {a₁ a₂ : G} {b₁ b₂ : k} :
  (single a₁ b₁ : add_monoid_star_algebra k G) * single a₂ b₂ = single (a₁ + a₂) (b₁ * b₂) :=
(sum_single_index (by simp only [zero_mul, single_zero, sum_zero])).trans
  (sum_single_index (by rw [mul_zero, single_zero]))
...
/-- Embedding of a monoid into its monoid algebra. -/
def of : multiplicative G →* add_monoid_star_algebra k G :=
{ to_fun := λ a, single a 1,
  map_one' := rfl,
...
lemma mul_single_apply_aux (f : add_monoid_star_algebra k G) (r : k)
  (x y z : G) (H : ∀ a, a + x = z ↔ a = y) :
  (f * single x r) z = f y * r :=
have A : ∀ a₁ b₁, (single x r).sum (λ a₂ b₂, ite (a₁ + a₂ = z) (b₁ * b₂) 0) =
  ite (a₁ + x = z) (b₁ * r) 0,
...
lemma mul_single_zero_apply (f : add_monoid_star_algebra k G) (r : k) (x : G) :
  (f * single 0 r) x = f x * r :=
f.mul_single_apply_aux r _ _ _ $ λ a, by rw [add_zero]

lemma single_mul_apply_aux (f : add_monoid_star_algebra k G) (r : k) (x y z : G)
  (H : ∀ a, x + a = y ↔ a = z) :
  (single x r * f) y = r * f z :=
have f.sum (λ a b, ite (x + a = y) (0 * b) 0) = 0, by simp,
calc (single x r * f) y = sum f (λ a b, ite (x + a = y) (r * b) 0) :
...
lemma single_zero_mul_apply (f : add_monoid_star_algebra k G) (r : k) (x : G) :
...
instance [comm_semiring k] [add_comm_monoid G] : comm_semiring (add_monoid_star_algebra k G) :=
{ mul_comm := assume f g,
  begin
    simp only [mul_def, finsupp.sum, mul_comm],
    rw [finset.sum_comm],
  ...
  .. add_monoid_star_algebra.semiring }

instance [ring k] : has_neg (add_monoid_star_algebra k G) :=
by apply_instance

instance [ring k] [add_monoid G] : ring (add_monoid_star_algebra k G) :=
{ neg := has_neg.neg,
  add_left_neg := add_left_neg,
  .. add_monoid_star_algebra.semiring }

instance [comm_ring k] [add_comm_monoid G] : comm_ring (add_monoid_star_algebra k G) :=
{ mul_comm := mul_comm, .. add_monoid_star_algebra.ring}

instance [semiring k] : has_scalar k (add_monoid_star_algebra k G) :=
finsupp.has_scalar

instance [semiring k] : semimodule k (add_monoid_star_algebra k G) :=
finsupp.semimodule G k
...
@[simp] lemma coe_algebra_map [comm_semiring k] [add_monoid G] :
  (algebra_map k (add_monoid_star_algebra k G) : k → add_monoid_star_algebra k G) = single 0 :=
rfl

/-- Any monoid homomorphism `multiplicative G →* R` can be lifted to an algebra homomorphism
`add_monoid_star_algebra k G →ₐ[k] R`. -/
def lift [comm_semiring k] [add_monoid G] {R : Type u₃} [semiring R] [algebra k R] :
  (multiplicative G →* R) ≃ (add_monoid_star_algebra k G →ₐ[k] R) :=
{ inv_fun := λ f, ((f : add_monoid_star_algebra k G →+* R) : add_monoid_star_algebra k G →* R).comp (of k G),
  to_fun := λ F, { to_fun := λ f, f.sum (λ a b, b • F a),
...
-- It is hard to state the equivalent of `distrib_mul_action G (monoid_star_algebra k G)`
